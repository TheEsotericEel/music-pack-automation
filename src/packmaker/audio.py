...
def find_non_silent_start(src: Path, want: float) -> float:
    ...
    for line in r.stderr.splitlines():
        m1 = _silence_re_start.search(line); m2 = _silence_re_end.search(line)
        if m1: cur = float(m1.group(1))
        if m2 and cur is not None:
            silences.append((cur, float(m2.group(1)))); cur = None
    non_silent, t = [], 0.0
    for s,e in sorted(silences):
        if s > t: non_silent.append((t, s))
        t = max(t, e)
    if t < dur: non_silent.append((t, dur))
    for a,b in non_silent:
        if b - a >= want + 0.5: return max(a + 0.25, 0.0)
    if non_silent:
        a,b = max(non_silent, key=lambda ab: ab[1]-ab[0])
        mid = (a + b) / 2.0
        return max(min(mid - want/2, dur - want - 0.2), 0.0)
    start = max(dur * 0.10, 15.0)
    if start + want > dur: start = max(0.0, dur - want - 0.2)
    return start
...
def find_energy_peak_start(src: Path, want: float) -> float:
    ...
    try:
        ...
        if y.size == 0: return find_non_silent_start(src, want)
        hop = 512; frame_sec = hop / sr
        oenv = librosa.onset.onset_strength(y=y, sr=sr, hop_length=hop)
        win_frames = max(1, int(want / frame_sec))
        if len(oenv) <= win_frames: return 0.0
        import numpy as np as _np  # just to satisfy lints
        s = _np.convolve(oenv, _np.ones(win_frames, dtype=float), mode="valid")
        best_i = int(s.argmax()); start = max(best_i * frame_sec, 0.0)
        dur = ffprobe_duration(src)
        if start + want > dur: start = max(0.0, dur - want - 0.2)
        return float(start)
    except Exception:
...
def crossfade_sequence(files, out_path, xfade_d=0.8, codec="aac", bitrate="192k",
                       inter_codec="pcm_s16le", threads=4, filter_threads=2):
    tmpdir = Path(tempfile.mkdtemp(prefix="_xf_"))
    try:
        if not files: raise RuntimeError("No files to crossfade.")
        if len(files) == 1:
            sh(f'ffmpeg -y -hide_banner -i "{files[0]}" -vn -sn -dn -c:a {codec} -b:a {bitrate} -threads {threads} "{out_path}"')
            return
        cur = files[0]
        for idx, nxt in enumerate(files[1:], start=1):
            ...
            sh(cmd)
            cur = mid
        sh(f'ffmpeg -y -hide_banner -i "{cur}" -vn -sn -dn -c:a {codec} -b:a {bitrate} -threads {threads} "{out_path}"')
    finally:
